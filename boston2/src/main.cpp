...
int main(int argc, char* argv[]) {
    ...
    int nCrimesSet = 0;
    CrimeSet * array_crimeset = AllocateArrayCrimeSet(nCrimesSet);

    string filename = argv[1];

    ifstream files_list (filename);

    ReadArrayCrimeSet(files_list, array_crimeset, nCrimesSet);
    ...
    for (int i=0; i < nCrimesSet; i++){
        array_crimeset[i].normalize();
        array_crimeset[i] = array_crimeset[i].selectValidLocation().selectWhereEQ("Shooting","1");
    }

    // Make the fusion of all the CrimeSets in the array
    CrimeSet selectedCrimes;
    for (int i=0; i < nCrimesSet;i++){
        selectedCrimes.join(array_crimeset[i]);
    }
    ...
    if (argc >= ARG_OUTPUT) {
        ...
        outputFileName = argv[ARG_OUTPUT-1];
    }
    
    selectedCrimes.setComment("Fusion of the crm files whose names are in the file: " + filename);
    selectedCrimes.save(outputFileName);
    
    DeallocateArrayCrimeSet(array_crimeset);
