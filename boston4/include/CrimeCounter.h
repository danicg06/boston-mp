...
private:
    ...
    /**
    * @brief Allocates memory for a dynamic matrix of frequencies with a capacity 
    * equals to @p nRows * @p nCols
    * @param nRows number of rows of the dynamic matrix. 
    * @param nCols number of columns of the dynamic matrix. 
    * Input parameter
    * @return A pointer to the allocated memory for the dynamic matrix. If 
    * @p nRows <= 0 || @p nCols <= 0 it returns nullptr
    */
    int** allocateMatrixFrequencies(int nRows, int nCols);
    
    /**
    * @brief Deallocates the memory pointed by the provided dynamic matrix of 
    * the implicit object. The pointer of the frequencies will also be set to 
    * nullptr
    */
    void deallocateMatrixFrequencies();
    
    /**
     * @brief Copies the frequencies (also the number of rows and columns) and 
     * the bottom left and top right corners of the input geographical 
