...
public:
    ...
    * with a size of INITIAL_CAPACITY and an empty comment
    * Modifier method
    */
    void clear();
    ...
    /**
    * @brief Appends the provided Crime @p newCrime at the array of the implicit 
    * object. This function starts checking if there is enough space. If not, it
    * extends the capacity up to @p _capacity*GROWING_RATIO
    * @param newCrime The new Crime to be inserted at the end of the
    * dynamic array. Input parameter
    */
    void append(const Crime &newCrime);
     ...
     * The newsize CrimeSet will have the same comments as this object plus a line
...
private:
   ...
   */
    int posMinArrayCrimes(int initialPos, int finalPos);
    
    /**
    * @brief Swaps the elements at positions @p first and @p second in _crimes
   ...
   */
    void swapElementsArrayCrimes(int first, int second);
    
    /**
    * @brief Allocates memory for a dynamic array of Crimes with a capacity 
    * equals to @p nCrimes
    * @param nCrimes number of CrimeSets to allocate in the dynamic array. 
    * Input parameter
    * @return A pointer to the allocated memory for the dynamic array. If 
    * @p nCrimeSets <= 0 it returns nullptr
    */
    Crime * allocateArrayCrime(int nCrimes);

    /**
    * @brief Deallocates the memory pointed by the provided dynamic array of 
    * the implicit object. That pointer will also be set to nullptr
    * @param arrayCrimes A pointer to a dynamic array of CrimeSet. 
    * The pointer is an input/output parameter and the content of the array is 
    * modified
    */
    void deallocateArrayCrime();
    
    /**
    * @brief Resizes the memory pointed by the provided dinamic array of
    * the implicit object.
    */
    void reallocateArrayCrime();
    
    /**
    * @brief Resizes the memory pointed by the provided dinamic array of
    * the implicit object.
    * @param nCrimes number of CrimeSets to reallocate in the dynamic array. 
    */
    void reallocateArrayCrime(int nCrimes);
    
    /**
     * @brief Copies the crimes and the comment of the @p CrimeSet object in 
     * the implicit object 
     * @param orig
     */
    void copy(const CrimeSet & orig);
    
    /**
     * @brief Clears the comment and resets to 0 the number of crimes of the implicit object. 
     */
    void clearNumComment();
};
